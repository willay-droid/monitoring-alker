...
export default function AdminLoginPage({
  ...
  const info = typeof sp.info === "string" ? sp.info : "";
  const nikPrefill = typeof sp.nik === "string" ? sp.nik : "";
      ...
      <div className="w-full max-w-md rounded-2xl border bg-white p-5 space-y-4">
        ...
        <form className="space-y-3">
          ...
          <button
            formAction={async (fd) => {
              ...
              if (!nik) redirect(`/admin-login?error=${encodeURIComponent("NIK wajib diisi.")}`);

              const res = await requestAdminOtp(fd);
              if (!res.ok) {
                redirect(`/admin-login?nik=${encodeURIComponent(nik)}&error=${encodeURIComponent(res.message)}`);
              }
              redirect(`/admin-login?nik=${encodeURIComponent(nik)}&info=${encodeURIComponent(res.message)}`);
            }}
          ...
          <button
            formAction={async (fd) => {
              "use server";
              const nik = String(fd.get("nik") ?? "").trim();
              if (!nik) redirect(`/admin-login?error=${encodeURIComponent("Isi NIK dulu lalu kirim OTP.")}`);
        ...
        <form
          action={async () => {
            "use server";
            await adminLogout();
            redirect("/admin-login?info=" + encodeURIComponent("Logout sukses."));
          }}
        >
